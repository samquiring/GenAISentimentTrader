...
class EntityExtractionSignature(dspy.Signature):
    ...
    content: str = dspy.InputField(desc="Text content to analyze for financial entities")
    symbols: list[str] = dspy.OutputField(desc="List of financial symbols found (e.g., ['AAPL', 'BTC'])")
    asset_types: list[Literal["STOCK", "CRYPTO", "ETF", "COMMODITY"]] = dspy.OutputField(desc="List of asset types corresponding to symbols")
    sentiments: list[Literal["POSITIVE", "NEGATIVE", "NEUTRAL", "MIXED"]] = dspy.OutputField(desc="List of sentiment for each entity")
    confidences: list[float] = dspy.OutputField(desc="List of confidence scores from 0.0 to 1.0")
    reasonings: list[str] = dspy.OutputField(desc="List of reasoning for sentiments")
    contexts: list[str] = dspy.OutputField(desc="List of relevant context around each entity mention")
...
class EntityExtractor:
    ...
    async def forward(self, content_item: ContentItem) -> List[ExtractedEntity]:
        ...
        for symbol, asset_type_str, confidence, context, sentiment_str, reasoning in zip(
            pred.symbols, pred.asset_types, pred.confidences, pred.contexts, pred.sentiments, pred.reasonings
        ):
