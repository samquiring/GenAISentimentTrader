...
from genaisentimenttrader.models.types import AssetType, ContentItem, ContentType, SentimentType
from genaisentimenttrader.processors.sentiment_pipeline import SentimentProcessingPipeline
...
def create_sample_content() -> tuple[list[ContentItem], list[dict]]:
    ...
    sample_data = [
        {
            "text": "Apple (AAPL) stock is soaring after their latest earnings report showed record revenue. The iPhone sales exceeded expectations.",
            "expected": {"symbols": ["AAPL"], "asset_types": [AssetType.STOCK], "sentiments": [SentimentType.POSITIVE]}
        },
        {
            "text": "Bitcoin (BTC) crashed today amid regulatory concerns. Many investors are selling their crypto holdings.",
            "expected": {"symbols": ["BTC"], "asset_types": [AssetType.CRYPTO], "sentiments": [SentimentType.NEGATIVE]}
        },
        {
            "text": "Tesla (TSLA) announced a new gigafactory in Texas. Elon Musk tweeted that production will begin next year.",
            "expected": {"symbols": ["TSLA"], "asset_types": [AssetType.STOCK], "sentiments": [SentimentType.POSITIVE]}
        },
        {
            "text": "Google parent Alphabet (GOOGL) reported strong Q4 results. Their cloud division grew 45% year-over-year.",
            "expected": {"symbols": ["GOOGL"], "asset_types": [AssetType.STOCK], "sentiments": [SentimentType.POSITIVE]}
        },
        {
            "text": "Oil prices (WTI) are rising due to geopolitical tensions in the Middle East. Energy stocks are benefiting.",
            "expected": {"symbols": ["WTI"], "asset_types": [AssetType.COMMODITY], "sentiments": [SentimentType.POSITIVE]}
        },
        {
            "text": "Netflix (NFLX) subscriber growth disappointed investors. The stock fell 8% in after-hours trading.",
            "expected": {"symbols": ["NFLX"], "asset_types": [AssetType.STOCK], "sentiments": [SentimentType.NEGATIVE]}
        },
        {
            "text": "Ethereum (ETH) hit a new all-time high as DeFi adoption continues to grow rapidly.",
            "expected": {"symbols": ["ETH"], "asset_types": [AssetType.CRYPTO], "sentiments": [SentimentType.POSITIVE]}
        },
        {
            "text": "Microsoft (MSFT) Azure revenue grew 50% as companies accelerate digital transformation initiatives.",
            "expected": {"symbols": ["MSFT"], "asset_types": [AssetType.STOCK], "sentiments": [SentimentType.POSITIVE]}
        }
    ]
...
async def test_basic_integration(create_sample_content):
    ...
    for i, result in enumerate(results):
        ...
        logger.info(f"  Expected: {expected['symbols']} ({[at.value for at in expected['asset_types']]}) - {[st.value for st in expected['sentiments']]}")
        logger.info(f"  Found {len(entities)} entities:")
        
        # Check if any extracted entities match expected ones
        expected_symbols = set(expected["symbols"])
        found_symbols = set(entity.symbol for entity in entities)
        
        for entity in entities:
            logger.info(f"    {entity.symbol} ({entity.asset_type.value}) - {entity.sentiment.value} (confidence: {entity.confidence:.2f})")
            logger.info(f"      Context: {entity.context[:80]}...")
            
            # Check if this entity matches expectations
            if (entity.symbol in expected_symbols and 
                entity.asset_type in expected["asset_types"] and
                entity.sentiment in expected["sentiments"]):
                correct_extractions += 1
                logger.info(f"      ✓ MATCHES EXPECTED")
            else:
                logger.info(f"      ✗ Does not match expected")
    
    accuracy = correct_extractions / max(len(expected_results), 1) if expected_results else 0
    logger.info(f"\n=== VALIDATION RESULTS ===")
...
async def test_single_item_processing():
    ...
    test_item = ContentItem(
        ...
        text="Amazon (AMZN) stock surged 12% after announcing record-breaking Prime Day sales. The e-commerce giant also reported strong AWS growth.",
