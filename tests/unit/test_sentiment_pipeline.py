...
from genaisentimenttrader.models.types import AssetType, ContentItem, ContentType, ExtractedEntity, ProcessingResult, SentimentType
...
class TestSentimentProcessingPipeline:
    ...
    @pytest.mark.asyncio
    async def test_process_single_item_success(self, pipeline, sample_content_items, sample_extracted_entities):
    ...
    async def test_process_single_item_failure(self, pipeline, sample_content_items):
        ...
        with patch('genaisentimenttrader.processors.sentiment_pipeline.logger') as mock_logger:
            result = await pipeline.process_single_item(content_item)
    ...
    @pytest.mark.asyncio
    async def test_process_batch_success(self, pipeline, sample_content_items, sample_extracted_entities):
    ...
    @pytest.mark.asyncio
    async def test_process_batch_with_exceptions(self, pipeline, sample_content_items, sample_extracted_entities):
    ...
    @pytest.mark.asyncio
    async def test_process_all_pending_single_batch(self, pipeline, sample_content_items, sample_extracted_entities):
        ...
        pipeline.data_sink.get_items.assert_called_once_with(10)  # Default large batch size
    
    @pytest.mark.asyncio
    async def test_process_all_pending_multiple_batches(self, pipeline, sample_content_items):
    ...
    async def test_run_continuous_exception_handling(self, pipeline):
        ...
        with patch('genaisentimenttrader.processors.sentiment_pipeline.logger') as mock_logger:
            try:
                await asyncio.wait_for(
                    pipeline.run_continuous(poll_interval=0.02),
                    timeout=1.0
                )
        ...
        assert any("Error in processing pipeline" in call or "Sink error" in call for call in error_calls)
    ...
    def test_stop(self, pipeline):
        ...
        assert "Stopping sentiment processing pipeline" in str(mock_logger.info.call_args)
